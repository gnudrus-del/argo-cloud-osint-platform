...
- **No independent external security audit has been performed.** An internal adversarial review of the cryptographic code (envelope encryption, both rotation CLIs) was done independently of the code that was reviewed, and found real, now-fixed issues (see `docs/THREAT_MODEL.md`'s "Known limitations" for the list). That is due diligence, not a substitute for a paid, professional, external review — the whole point of this document is to make commissioning one easier, not to claim one already happened.
...
| Job | What it checks |
| --- | --- |
| `python` | Lint (ruff, blocking), unit tests (709 at time of writing, `python -m unittest discover -s tests`), JS syntax |
