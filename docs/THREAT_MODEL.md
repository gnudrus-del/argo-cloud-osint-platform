...
- **Connector API keys are encrypted at rest** (envelope encryption, `osint_bot/secrets_crypto.py`): a random per-secret data key encrypts the value (AES-256-GCM), wrapped by an instance master key that lives outside the database (env var, systemd-credential file, or an auto-generated local file, never a DB row, never `.env`). Every value is bound via AES-GCM associated data to its own `(username, service)` row, so a direct DB-*write* attacker (SQL injection, a rogue script — a different threat than a read-only DB leak) can't relocate one row's ciphertext into another and have it decrypt there. This protects against a database-only leak — a stolen SQLite file or Postgres dump. It does **not** protect against a compromise of the host the master key file lives on (see "Compromise of the host OS" above — that remains out of scope by design, same as everything else here). Rotate with `argo-rotate-master-key` — **stop the web service first**: the rotation CLI takes one snapshot and has no lock against a concurrent writer.
...
- **Ed25519 signing-key rotation exists** (`argo-rotate-signing-key`) — retires the current key (archived on disk, never deleted) and generates a new one; past signatures stay verifiable unchanged since every seal embeds its own public key. As with the API-key master key, a running service must be **restarted** after rotation — it caches the signing key in memory with no cross-process invalidation.
- **An internal adversarial review of this session's cryptographic code** (envelope encryption, signing-key rotation, both rotation CLIs) was performed by an independent context with no memory of writing it. It found and this codebase fixed: a rotation-ordering bug that could have permanently lost API keys on a crash mid-rotation, a false "no restart needed" claim in the signing-key rotation CLI, a low-probability archive-filename collision in concurrent signing-key rotations, missing row-binding on the ciphertext (the AAD fix above), and silently-swallowed file-permission failures. **This is not a substitute for an external, professional security audit** — see `docs/AUDIT_READINESS.md` — it is disclosed here specifically so that claim is never conflated with one.
